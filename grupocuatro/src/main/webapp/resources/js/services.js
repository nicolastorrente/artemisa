...
function loadListsFrom(userId, loadItemsList) {
	...
	if (app.model.loggedUser.id == userId) {
		...
		$("#labelPanelLists").text(
				"Listas de " + app.model.userSelected.username);
	}
	$.ajax({
		type : "GET",
...
function showLists(user, ajaxresult, loadItemsList) {
	...
	for ( var k in ajaxresult) {
		if (loadItemsList && listId == 0) {
			...
			row = $("#rowTemplateListsActive").html().replace("{{LIST}}",
					ajaxresult[k].name)
					.replace("{{ID_LIST}}", ajaxresult[k].id);
		} else {
			row = $("#rowTemplateLists").html().replace("{{LIST}}",
					ajaxresult[k].name)
					.replace("{{ID_LIST}}", ajaxresult[k].id);
		}
...
function addList(data) {
	var row = $("#rowTemplateLists").html().replace("{{LIST}}", data.name)
			.replace("{{ID_LIST}}", data.id);
	$("#listOfList").append(row);
	$('#listOfList a').click(clickList);
	data.items = {};
...
function showItemsFrom(listId, items) {
	...
	for ( var k in items) {
		row = $("#rowTemplateItems").html()
				.replace("{{VOTES}}", items[k].votes).replace("{{ITEM}}",
						items[k].label).replace("{{ID_ITEM}}", items[k].id);
...
function addItem(data) {
	var row = $("#rowTemplateItems").html().replace("{{VOTES}}", data.votes)
			.replace("{{ITEM}}", data.label).replace("{{ID_ITEM}}", data.id);
	$("#listOfItems").append(row);
	$('#listOfItems a').click(selectItem);
...
$('#AgregarLista').on('click', function() {
	if ($('#lista_nombre').val() == "") {
		...
		$.ajax({
			...
			error : function(jqXHR, textStatus, errorThrown) {
				if (jqXHR.status == 404) {
					$('#alertListErrorMessage').text('Usuario Inexistente');
				} else if (jqXHR.status == 406) {
					$('#alertListErrorMessage').text('La lista ya existe!');
				} else if (jqXHR.status == 403) {
					$('#alertListErrorMessage').text('No podes crearle una lista a un amigo!');
				} else {
					$('#alertListErrorMessage').text('Error desconocido.');
				}
				$('#lista_nombre').val("");
				$('#alertListError').slideDown("slow");
				$('#lista').modal('hide');
			}
		});
	}
...
$('#AgregarItem').on(
		...
		function() {
			if ($('#item-label').val() == "") {
				...
				$.ajax({
					url : "/users/" + app.model.userSelected.id + "/lists/"
							+ app.model.selectedList.id + "/items",
					type : "POST",
					...
					if (app.model.userSelected.id != user_id) {
						...
						if (app.model.selectedList) {
							$
									.ajax({
										url : "/users/"
												+ app.model.userSelected.id
												+ "/lists/"
												+ app.model.selectedList.id, // 12341
																				// lista
																				// harcodeada
										type : "DELETE",
										success : function(data, textStatus,
											...
											app.model.selectedList = null;
											showLists(
													app.model.userSelected,
													app.model.userSelected.lists,
													false);
										},
		...
		function(e) {
			if (app.model.selectedItem) {
				$.ajax({
					url : "/users/" + app.model.userSelected.id + "/lists/"
							+ app.model.selectedList.id + "/items/"
							+ app.model.selectedItem.id + "/vote",
					type : "PUT",
					success : function(data, textStatus, jqXHR) {
						app.model.selectedItem.votes++;
						app.model.selectedItem.voters
								.push(app.model.userSelected.id);
